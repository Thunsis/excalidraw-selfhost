...
- **`excalidraw` CLI** — generate scenes straight from the terminal: `excalidraw ai "payment flow"` or `excalidraw mermaid flow.mmd`. Pure Node, same official conversion pipeline as the frontend AI (jsdom + `@excalidraw/mermaid-to-excalidraw` + `@excalidraw/element`), no playwright, no browser. CJK handwritten fonts & rough layout identical to the in-app AI.
...
npm install
npm link                     # → `excalidraw` on PATH (uses EXCALIDRAW_WS_USER / WS_API)
```
...
excalidraw ai "用户登录流程" --name "login flow"              # LLM → mermaid → scene (official pipeline)
excalidraw mermaid flow.mmd --name "my diagram"              # mermaid file → scene
...
flowchart LR
    ...
    CLI["excalidraw CLI<br/>pure Node · official pipeline"] -->|"WS_API"| WS
    CLI -->|"AI_API"| AI
...
## CLI — drawings from the terminal

The `excalidraw` CLI creates cloud scenes without opening the browser. `excalidraw ai "<prompt>"` asks the AI server for a Mermaid diagram (same as the in-app AI), then converts it; `excalidraw mermaid file.mmd` skips the LLM and converts a Mermaid file directly.

<img src="https://raw.githubusercontent.com/Thunsis/excalidraw-selfhost/main/docs/cli-example.png" alt="Scene generated by the CLI (flowchart, CJK handwritten font)" width="560">

<sup>Scene generated by the CLI — `excalidraw mermaid` → official pipeline → saved to the cloud workspace.</sup>
...
excalidraw export <id> [out.json]            export elements JSON
excalidraw library get|put <file.json>       cloud library
excalidraw doctor                            env & connectivity check
```

### Pure-Node official pipeline

No playwright, no browser — the CLI runs the **same conversion chain as the frontend AI**:

```mermaid
flowchart LR
    M["Mermaid text"] --> R["mermaid official render<br/>(jsdom env)"]
    R --> M2E["@excalidraw/mermaid-to-excalidraw<br/>official parser (fixed 2.2.2)"]
    M2E --> E["@excalidraw/element<br/>convertToExcalidrawElements<br/>(same build as frontend)"]
    E --> S["scene saved via ws-server"]
```

- **jsdom** is the only environment shim — the same environment mermaid's own CI uses. Layout, fonts, bindings and z-order are decided 100% by the official packages.
- **Text measurement is real font data**: `cli/src/metrics.js` reads the system Trebuchet MS (mermaid's default stack) hmtx table via opentype.js — the same metrics the browser's `getBBox` returns. No magic coefficients.
- **Same CJK handwriting as the frontend** — the patched frontend preloads the Xiaolai font slices (`Fonts.loadElementsFonts` after `updateScene`), so scenes open with the handwritten look in-browser too.
...
| sequenceDiagram | ⚠️ converts; edge points null (official converter limitation) | ⚠️ same |
| classDiagram / erDiagram / stateDiagram / gantt | ⚠️ rejected with clear error (official converter falls back to graphImage in browsers too) | ⚠️ same fallback |
...
The pure-Node output is verified element-by-element against a real-browser run (via mcphub → playwright MCP, verification only): shape sizes within ~4%, y-range within ~2%.
...
cli/             excalidraw CLI (pure-Node official pipeline, tests/)
